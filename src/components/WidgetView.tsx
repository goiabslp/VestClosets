...
export const WidgetView: React.FC = () => {
  ...
  // 🌟 NOVO: Estado para guardar qual botão o usuário clicou
  const [selectedCategory, setSelectedCategory] = useState<'upper_body' | 'lower_body' | 'dresses'>('upper_body');
    ...
    const carregarImagemDaUrl = async () => {
      ...
      if (urlDaRoupa) {
        try {
          ...
          console.error("Erro ao processar imagem da loja:", error);
        }
      }
    };
    carregarImagemDaUrl();
      ...
      {!resultImage ? (
        <>
          ...
          <div className="w-full max-w-sm mt-4">
            ...
            {/* 🌟 NOVO: Os Botõezinhos de Categoria */}
            <div className="flex gap-2 mb-4">
              <button
                onClick={() => setSelectedCategory('upper_body')}
                className={`flex-1 py-2 text-[10px] font-bold uppercase tracking-wider rounded border transition-colors ${selectedCategory === 'upper_body' ? 'bg-[#525252] text-white border-[#525252]' : 'bg-white text-gray-500 border-gray-200 hover:bg-gray-50'}`}
              >
                Blusa
              </button>
              <button
                onClick={() => setSelectedCategory('lower_body')}
                className={`flex-1 py-2 text-[10px] font-bold uppercase tracking-wider rounded border transition-colors ${selectedCategory === 'lower_body' ? 'bg-[#525252] text-white border-[#525252]' : 'bg-white text-gray-500 border-gray-200 hover:bg-gray-50'}`}
              >
                Calça/Saia
              </button>
              <button
                onClick={() => setSelectedCategory('dresses')}
                className={`flex-1 py-2 text-[10px] font-bold uppercase tracking-wider rounded border transition-colors ${selectedCategory === 'dresses' ? 'bg-[#525252] text-white border-[#525252]' : 'bg-white text-gray-500 border-gray-200 hover:bg-gray-50'}`}
              >
                Vestido
              </button>
            </div>
            ...
            <button
              // 🌟 NOVO: Enviando a categoria escolhida para o Hook
              onClick={() => sendTryOnRequest(selectedCategory)}
        ...
        <div className="w-full max-w-sm flex flex-col items-center">
          <div className="text-center mb-6">
            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Resultado</span>
          ...
          <div className="w-full aspect-[3/4] bg-gray-100 rounded-lg overflow-hidden shadow-inner mb-6 relative">
            <img alt="Resultado IA" className="w-full h-full object-contain" src={resultImage} />
          </div>
            ...
            <button
              onClick={async () => {
                try {
                  ...
                  link.href = blobUrl; link.download = `digital-atelier-${Date.now()}.png`;
                  document.body.appendChild(link); link.click(); document.body.removeChild(link); URL.revokeObjectURL(blobUrl);
                } catch (error) {
                  const link = document.createElement('a');
                  link.href = resultImage; link.download = `digital-atelier-${Date.now()}.png`;
